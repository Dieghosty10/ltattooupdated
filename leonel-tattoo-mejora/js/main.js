(function(){
  ...
  const phone = (window.APP && window.APP.phone) || (window.APP_CONFIG && window.APP_CONFIG.phone) || "";
  const yearEl = $("#year");
  if (yearEl) yearEl.textContent = new Date().getFullYear();
  // Construir url de WhatsApp con texto opcional
  ...
  // Actualizar enlaces de WhatsApp en la página (footer, flotante, botones)
  function updateWhatsAppLinks(){
    const waAnchors = Array.from(document.querySelectorAll('a[href*="wa.me"], a#footer-wa-link, a.floating-whatsapp'));
    waAnchors.forEach(a=>{
      if(a.dataset && a.dataset.prefill) {
        a.href = buildWhatsAppUrl(a.dataset.prefill);
      } else {
  ...
  // Interceptar envío del formulario de reserva para abrir WhatsApp con mensaje prellenado
  ...
  if(bookingForm){
    bookingForm.addEventListener('submit', function(e){
      ...
      const message = `Hola, soy ${name}. Me gustaría cotizar un tatuaje. Presupuesto: ${budget}. Detalles: ${details}. Email: ${email}.`;
